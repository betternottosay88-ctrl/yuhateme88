...
export default async function fnWYVKWVQT(req, res) {
  if (!TARGET_BASE) {
    res.statusCode = 500;
  ...
  try {
    ...
    const hasBody = method !== "\x47\x45\x54" && method !== "\x48\x45\x41\x44";

    const fetchOpts = { method, headers, redirect: "\x6d\x61\x6e\x75\x61\x6c" };
    if (hasBody) {
